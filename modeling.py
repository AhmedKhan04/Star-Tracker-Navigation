...
class StarModeling:
    ...
    def __init__(self, tuples_values):
        self.lightcurve_tuple_list = tuples_values



    
    def compGetPeriodogramData(self, nameOfStar): 
        """
    ...
    def sine_model(self, t, amplitude, phase, frequency, offset):
        """
    ...
    def identifyPeaks_dynamic(self, nameOfStar, lowerscalar = 0.1):
        """
        ...
        """
        pg, lightc = self.compGetPeriodogramData(nameOfStar)
    ...
    def identifyPeaks_non_dynamic(self, nameOfStar):
        """
        ...
        """
        pg, ltcurves = self.compGetPeriodogramData(nameOfStar)
        max_power = np.max(pg.power.value)
    ...
    def guess_swift(self, nameOfStar,bounds1,search_result, frequencyfitted):
        """
        ...
        while b < len(frequencyfitted):
            ...
            params, _ = curve_fit(self.sine_model, time, flux, p0=ig, bounds=bounds, maxfev=999999999, method='dogbox')
    ...
    def guess_deep(self, a, scalar, frequencyfitted, search_result, powers):
        """
    ...
    def align_arrays(self, time, flux):
        """
    ...
    def getMeanSquaredResidual(self, nameOfStar, search_result, frequency, powerofpeaks_arg):
            """
            ...
            for bounds1 in range(54,56): 
                listofsines = self.guess_swift(nameOfStar,bounds1, search_result, frequency)
                addedTogether = 0
                time = lc.time.value
                flux = lc.flux.value
                #flux_err = np.mean(lc.flux_err.value)
                time, flux = self.align_arrays(time,flux)
                powerOfPeaks = powerofpeaks_arg
                powerOfPeaks = powerOfPeaks.value
                p = 0 
                total_weight = 0 
                while (p < len(powerOfPeaks)):
                    sinInterpolated = self.interpolate(time, listofsines[p], time)
    ...
    def getResiduals(self, fit, flux): 
        """
    ...
    def getCompositeSine2_swift(self, nameOfStar):
            """
            ...
            powerOfPeaks = powerOfPeaks.value
            listofsines, lc = self.guess_baseline(nameOfStar)
            addedTogether = 0
            time = lc.time.value
            flux = lc.flux.value
            time, flux = self.align_arrays(time,flux)
            ...
            sine_print_terms = []
            print("Generating Composite Sine Function...")
            while (p < len(powerOfPeaks)):
    ...
    def getCompositeSine2_deep(self, nameOfStar):
            """
            ...
            print(len(powerOfPeaks))
            powerOfPeaks = powerOfPeaks.value
            frequencyfitted2, search_result2, powers2 = self.identifyPeaks_dynamic(nameOfStar)
            amplitude_scale = 0.5
            ...
            while (p < len(listofsines)):
                ...
                addedTogether += (weight/total_weight) * sinInterpolated
                p+=1
            bestmean = self.getResiduals(addedTogether, flux)
            ...
            while(bestFitAchieved == False): 
                ...
                lower, _,fits_low = self.guess_deep(nameOfStar, low_amplitude_scale, frequencyfitted2, search_result2, powers2)
                upper, _, fits_high = self.guess_deep(nameOfStar, high_amplitude_scale, frequencyfitted2, search_result2, powers2)
                lowertot = 0
                uppertot = 0 
                countinner = 0
                ...
                fit_low_MSE = self.getResiduals(lowertot, flux)
                fit_high_MSE = self.getResiduals(uppertot, flux)
    ...
    def plotsidebyside_swift(self, nameOfStar):
        """
        ...
        """
        print("Generating Swift Model...")
        function, lc = self.getCompositeSine2_swift(nameOfStar)
        flux = lc.flux.value
        print(flux)
        print(function)
        time = lc.time.value
    ...
    def plotsidebyside_deep(self, nameOfStar):
        """
        ...
        """
        function, lc, _ = self.getCompositeSine2_deep(nameOfStar)
        flux = lc.flux.value
        print(flux)
        print(function)
        time = lc.time.value
    ...
    def interpolate(self, time, flux, target_time):
        """
    ...
    def get_epsilon_value(self, star_name, sine_string):
        """
    ...
    def guess_baseline(self, nameOfStar):
        ...
        """
        frequencyfitted, search_result, powers = self.identifyPeaks_dynamic(nameOfStar)
        lc = search_result
        b = 0 
        c = []
        params_list = []
        ...
        while b < len(frequencyfitted):
            ...
            if len(time) == 0 or len(flux) == 0:
                raise ValueError("After cleaning, the time or flux array is empty.")
            #ig = [(np.max(flux) - np.min(flux))/2, 0, frequencyfitted[b].value, np.mean(flux)]
            params, _ = curve_fit(self.sine_model, time, flux, p0=ig, bounds=bounds, method='dogbox', maxfev = 9999999)
            amplitude, phase, frequency, offset = params
            fit_c = self.sine_model(time, *params)
            c.append(fit_c)
            params_list.append((amplitude, phase, frequency, offset)) 
    ...
    def getCompositeSine(self, nameOfStar):
            """
            ...
            """
            listofsines = self.guess_baseline(nameOfStar)
            addedTogether = 0
            ...
            flux = lc.flux.value
            time, flux = self.align_arrays(time,flux)
            powerOfPeaks = self.identifyPeaks_non_dynamic(nameOfStar).value
    ...
    def get_csv_epsilon_value(self, csv_file_path): 
        """
        ...
        try:
            ...
            while( i < len(KIC_list)):
                eps, sig, m, P_max = self.get_epsilon_value(KIC_list[i], FUNCTION_list[i])
                if(i>0):
                    pt.close('all')
    ...
    def find_valid_segments(self, all_time, all_flux, dt=1.0, t_step=0.1, expected_cadence_days=1800/86400):
        """
    ...
    def seriesofstars_deep(self, listofstars):
        """
        ...
        try:
            for star in listofstars:
                print(f"KIC {star}")
                function, lc, composite_strings = self.getCompositeSine2_deep(f"KIC {star}")
                if (function[0] == -10):
                    continue
                flux = lc.flux.value
                print(flux)
    ...
    def SpectralResiduals(self, nameOfStar, sine_string): 
        """
    ...
    def SpectralResidualsCsvBased(self, csv_file_path): 
        """
        ...
        try:
            ...
            master_list_eps = []
            while( i < len(KIC_list)):
                spectral_resid, R2 = self.SpectralResiduals(KIC_list[i], FUNCTION_list[i])
                if(i>0):
                    pt.close('all')
    ...
    def cleaning_tess(self, csv_path):
        """
    ...
    def cleaning_tess_plotting(self, csv_path):
        """
        Given a csv containing KIC, TIC and predictive models for a series of stars, cleans the corresponding TESS data 
