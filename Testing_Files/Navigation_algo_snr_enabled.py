...
def monte_carlo_simulation(n_runs=50, verbose=True):
    ...
    num_stars = np.random.randint(4, 100)  # Randomly choose between 10 and 100 stars
    num_stars_array = []
    for i in range(n_runs):
        num_stars_array.append(np.random.randint(4, 100))
    ...
    obs_duration = 5  # days
    n_samples = 500
    noise_sigma_mean = 1e-3 #from paper 
    ...
    for i, run in enumerate(range(n_runs)):
        stars = []
        for j in range(num_stars_array[i]):
            freq = np.random.uniform(5, 15)  # cycles per day
            amp = 0.01  # 1% variation
            phase = np.random.uniform(0, 2*np.pi)
            baseline = 1.0
        ...
        #noise_sigma = noise_sigma_mean * np.random.uniform(0.5, 100)  # Vary noise level slightly
    ...
    plt.figure()
    plt.plot(num_stars_array, position_errors, 'bo', alpha=0.5)
    plt.xlabel('Number of Stars')
    plt.ylabel('Position Error (AU)')
    plt.title('Monte Carlo Simulation Results ($\sigma^2$ = 1e-3, n_runs = 500)')
    plt.show()
    return {
...
def monte_carlo_simulation_snr(n_runs=50, verbose=True):
    ...
    if success_count > 0:
        ...
        print(f"  Yours - Time:     mean={np.mean(time_errors):.2f} s, std={np.std(time_errors):.2f} s")
    plt.figure()
    plt.plot(noise_sigmas, position_errors, 'bo', alpha=0.5)
    plt.xlabel('Gaussian Noise $\sigma^2$')
    plt.ylabel('Position Error (au)')
    plt.title('Monte Carlo Simulation Results (n stars = 10, n runs = 500)')
    plt.show()
    return {
        'position_errors': position_errors,
        'time_errors': time_errors,
...
def main():
    ...
    # Monte Carlo Testing
    monte_carlo_simulation(verbose=False, n_runs = 500)
